      ...
      <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-2 gap-10">
        ...
        <section className="p-8 border border-white/10 rounded-2xl shadow-sm bg-white/5">
          ...
          <p className="text-sm text-gray-400 mb-6">
            Have a question about an order or our products? Leave us a message and we'll get back within 24–48 hours.
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                ...
                <label key={field.name} className="flex flex-col">
                  <span className="text-sm mb-1 text-gray-400">{field.label}</span>
                  <input
                    name={field.name}
            ...
            {[
              { label: "Order number (optional)", name: "order", type: "text", placeholder: "#12345" },
            ...
            <div className="flex items-center gap-3">
              ...
              {status === "success" && <span className="text-sm text-gray-400">Thanks — we'll reply soon.</span>}
              {status === "error" && <span className="text-sm text-red-400">Something went wrong. Try again.</span>}
            </div>

            <p className="text-xs text-gray-400 mt-2">
              By sending this message, you agree to our <a className="underline" href="#">privacy policy</a>.
            </p>
          </form>
        </section>
        ...
        <aside className="p-8 border border-white/10 rounded-2xl shadow-sm bg-white/5 flex flex-col justify-between">
          <div>
            <h2 className="text-xl font-medium mb-3 text-gray-300">Other ways to reach us</h2>

            <div className="space-y-4 text-sm text-gray-400">
              <ContactRow icon={<IconMail />} title="Email" text="support@yourecommercestore.com" />
              <ContactRow icon={<IconPhone />} title="Phone" text="+1 (555) 123-4567" />
              <ContactRow icon={<IconClock />} title="Support hours" text="Mon — Fri: 9:00 — 18:00 (UTC)" />
              <ContactRow icon={<IconLocation />} title="Address" text="123 Commerce St, Suite 101, Cityname, Country" />
            </div>

            <hr className="my-6 border-white/10" />

            <h3 className="text-sm font-semibold mb-3 text-gray-300">Quick answers</h3>
            <dl className="text-sm space-y-3 text-gray-400">
              <FAQ q="Where is my order?" a="Use the tracking link sent to your email, or paste your order number above and ask us to look it up." />
              <FAQ q="Returns & exchanges" a="We accept returns within 30 days — items must be unused. See our returns page for details." />
              <FAQ q="Bulk / wholesale" a="Email wholesale@yourecommercestore.com for pricing and availability." />
            </dl>
          </div>

          <div className="mt-6">
            <div className="text-sm font-medium mb-2 text-gray-300">Store location</div>
...
function IconClock() {
  return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="9" /><path d="M12 7v6l4 2" /></svg>;
}
function IconLocation() {
  return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 12-9 12S3 17 3 10a9 9 0 1 1 18 0z" /><circle cx="12" cy="10" r="2.5" /></svg>;
}
function SocialIcon({ children, label }) {
  return (
    <div className="flex items-center justify-center w-10 h-10 border border-white/10 rounded-md text-sm font-medium text-white" title={label} aria-label={label}>
      {children}
    </div>
  );
}
